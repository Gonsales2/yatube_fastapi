...
class PostUseCase:
        ...
        if post_in.group is not None:
            ...
            if not group:
                raise ValidationError(
                    field="group",
                    message=f"Группа с id={post_in.group} не существует"
                )

        if len(post_in.text.strip()) < 3:
            raise ValidationError(
                field="text",
                message="Текст поста должен содержать минимум 3 символа"
            )
    ...
    def update_post(self, user: User, post_id: int, post_in: PostUpdate, full_update: bool = False) -> dict:
        ...
        if full_update and post_in.text is not None and not post_in.text.strip():
            raise ValidationError(
                field="text",
                message="Поле text обязательно для полного обновления"
            )
